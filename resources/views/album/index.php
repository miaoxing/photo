...
<?= $block('css') ?>
<link rel="stylesheet" href="<?= $asset(['assets/buttonsRow.css', 'assets/apps/album.css', 'comps/TouchNSwipe/css/touchnswipe.min.css', 'comps/TouchNSwipe/css/fullscreen_popup.css']) ?>"/>
<?= $block->end() ?>

<div class="buttons-row album-buttons-row">
  <?php foreach ($categories as $category) : ?>
    <a href="<?= $url('album', ['categoryId' => $category['id']]) ?>" class="button tab-link <?= $currCategory['id'] == $category['id'] ? 'active' : '' ?>"><?= $category['name'] ?></a>
  <?php endforeach ?>
</div>
...
<div class="sliderHolder" data-elem="sliderHolder">
  <div class="slider" data-elem="slider" data-options="initShow:false; resetScrollDuration:1;" data-show="autoAlpha:1; display:block" data-hide="autoAlpha:0; display:none">
    ...
    <div class="slides" data-elem="slides" data-options="preloaderUrl:/comps/TouchNSwipe/assets/preloader.gif; resizeDuration:1; adjustHeight:-30"></div>
    <div class="captionHolder" data-elem="captionHolder">
      <div class="caption blackBgAlpha60" data-elem="caption" data-options="initShow:true; setHolderHeight:true; resizeDuration:1;" data-show="top:0%; display:block; autoAlpha:1;"
